          ...
          <Col span="12">
            ...
            <div class="org-item" v-for="(item, index) in distributeInfoList" :key="index">
              <div class="org-title">
                ...
                <div class="right">
                  <span class="red" v-if="!item.ifOperation" @click="handleRmove(index, item)">
                    <i class="iconfont icon-yichu-1"></i>移除</span
                  >
                </div>
              </div>
              <div class="bottom">
...
export default {
  ...
  methods: {
    ...
    // 保存并分发
    handleSave() {
      if (this.distributeInfoList.length <= 0) {
        this.$Message.info('征求部门不能为空');
        return;
      }
      const data = {
        deptList: this.distributeInfoList,
