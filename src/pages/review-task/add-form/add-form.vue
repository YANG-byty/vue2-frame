...
export default {
  ...
  methods: {
    ...
    changeDate(e) {
      ...
      // this.$refs.endTime.onSelectionModeChange('date');
      this.$set(this.formValidate, 'endTime', moment(new Date(e)).format('YYYY-MM-DD'));
      this.ruleDate();
    },
    ruleDate() {
      if (this.formValidate.endTime) {
        if (this.formValidate.endTime <= moment(new Date()).format('YYYY-MM-DD')) {
